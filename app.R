...
server <- function(input, output, session) {
  ...
  raw_data <- reactive({
    ...
    if (!is.null(input$remove_row) && !is.na(input$remove_row)) {
      if (input$remove_row >= 1 && input$remove_row <= nrow(df)) { df <- df[-input$remove_row, ] }
    }
    return(df)
  })
    ...
    for(i in 3:ncol(df)) {
      ...
      if(length(active_vals) > 0) {
        ...
        if(!is.na(cpk_v)) {
          ...
          if(!is.na(match_idx)) {
            # Offset: 3 summary + 2 limit + 1 header = 6. (DT index is 0-based for rows)
      ...
      showModal(modalDialog(
        ...
        fluidRow(
          column(6, renderPlotly({
            p1 <- ggplot(data.frame(x=col_data$raw), aes(x=x)) + geom_histogram(fill="steelblue", color="white") + 
              geom_vline(xintercept=c(col_data$l, col_data$h), color="red", linetype="dashed") + theme_minimal()
            ggplotly(p1)
          })),
          column(6, renderPlotly({
            p2 <- ggplot(data.frame(s=1:length(col_data$raw), v=col_data$raw), aes(x=s, y=v)) + geom_line() + geom_point() +
              geom_hline(yintercept=c(col_data$l, col_data$h), color="red", linetype="dashed") + theme_minimal()
            ggplotly(p2)
          }))
